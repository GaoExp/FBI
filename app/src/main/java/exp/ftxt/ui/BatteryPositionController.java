...
public class BatteryPositionController {
    ...
    private final PresetHandler.Delegate delegate = new PresetHandler.Delegate() {
        @Override
        public String moduleLabel() { return "Battery Info"; }
        @Override
        ...
        public void saveToPreset(OverlayPreset p) {
            ...
            p.showVoltage = BatteryStatsConfig.showVoltage;
            p.showCurrent = BatteryStatsConfig.showCurrent;
            p.showPower = BatteryStatsConfig.showPower;
            p.itemOrder = BatteryStatsConfig.itemOrder;
        }
        ...
        public void applyFromPreset(Activity activity, OverlayPreset p, SharedPreferences prefs) {
            ...
            if (p.showVoltage != null) {
                BatteryStatsConfig.showVoltage = p.showVoltage;
                prefs.edit().putBoolean("battery_show_voltage", BatteryStatsConfig.showVoltage).apply();
            }
            if (p.showCurrent != null) {
                BatteryStatsConfig.showCurrent = p.showCurrent;
                prefs.edit().putBoolean("battery_show_current", BatteryStatsConfig.showCurrent).apply();
            }
            if (p.showPower != null) {
                BatteryStatsConfig.showPower = p.showPower;
                prefs.edit().putBoolean("battery_show_power", BatteryStatsConfig.showPower).apply();
            }
            if (p.itemOrder != null && !p.itemOrder.isEmpty()) {
                BatteryStatsConfig.itemOrder = p.itemOrder;
                prefs.edit().putString("battery_item_order", BatteryStatsConfig.itemOrder).apply();
            }
        }
